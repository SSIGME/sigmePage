...
const EquipoDetail = () => {
    ...
    input.onchange = async (event) => {
      ...
      try {
        const response = await axios.post(
          `http://localhost:5000/upload_pdf/PALM/${codigoIdentificacion}`,
          formData
        );
  ...
  const checkDocuments = async (codigoIdentificacion: string) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/equipoDocuments/PALM/${codigoIdentificacion}`
      );
      if (response.status === 200) {
        setDocuments(response.data);
        console.log("Documentos:", response.data); // Verifica la respuesta
  ...
  const getEquipo = async (codigoIdentificacion: string) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/equipo/PALM/${codigoIdentificacion}`
      );
      if (response.status === 200) {
